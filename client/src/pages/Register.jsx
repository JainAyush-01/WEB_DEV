...
const userRegistrationSchema = z.object({
  ...
  collegeId: z.string().regex(/^\d{2}(ucs|uec|ucc|ume|dcs|dec)\d{3}$/i, "Roll No format must be like 24ucs001 (ucs/uec/ucc/ume/dcs/dec)").toLowerCase(),
  ...
  const validate = (field, value) => {
    ...
    switch (field) {
      ...
      case 'collegeId':
        ...
        fieldSchema = z.string().regex(/^\d{2}(ucs|uec|ucc|ume|dcs|dec)\d{3}$/i, "Roll No must be 2 digits + branch (ucs/uce/ucc/ume/dcs/dec) + 3 digits");
        break;
      case 'email':
